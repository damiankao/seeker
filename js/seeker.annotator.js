	...
	seeker.annotator = function() {
						...
						obj
							...
							.on('click', function(evt) {
								...
								if (seeker.env_clickTarget !== this) {
									...
									legendMenu
										.place(d3.mouse(base.container.node()))
										.show();
	    ...
	    var sequenceMenuData = [
	        {'name':'hide this sequence','click':function() {
	            seeker.util.set(base.targetData, 'show', false);
	            base.update();
	            seeker.env_closeAll();
	        }},
	        {'name':'show all features','click':function() {
	            var feats = base.targetData.feat;
	            var i = feats.length;
	            while ( i-- ) {
	                feats[i].show = true;
	            }
	    ...
	    var featureMenuData = [
	        {'name':'hide this feature','click':function() {
	            base.targetData.show = false;
	            base.update()
	            seeker.env_closeAll();
	        }},
	        {'name':'hide this label','click':function() {
	            base.targetData.label = false;
	            base.update()
	            seeker.env_closeAll();
	        }},
	        {'name':'show this label','click':function() {
	            base.targetData.label = true;
	            base.update()
	            seeker.env_closeAll();
	        }},
	        {'name':'show all features','click':function() {
	            var name = base.targetData.ref.name;
	    ...
	    var legendMenuData = [
	        {'name':'hide this legend','click':function() {
	            base.targetData.legend = false;
	            base.update()
	            seeker.env_closeAll();
	        }},
	        {'name':'show all features','click':function() {
	            var name = base.targetData.name;
	            base.showAllFeature(name);
	            seeker.env_closeAll();
	        }},
	        {'name':'hide all features','click':function() {
	    ...
	    var menu_sequences = new seeker.complexMenu()
	        ...
	        .setTemplate(function(li, data, index, keys) {
	            ...
	            d3.select(li)
	                .on('mouseover', function() {
	                    base.targetData = data.feat;
	                    submenu_features
	                        .reinit()
	                        .bind({'items':base.targetData}, {'text':'name','cb':'show'})
	                        .update()
	                        .show();
	    ...
	    var submenu_featuresControlData = [
	        {'name':'show all','click':function() {
	            var i = base.targetData.length;

	            while ( i-- ) {
	                seeker.util.set(base.targetData[i], 'show', true);
	            }
	        ...
	        {'name':'hide all','click':function() {
	            ...
	            while ( i-- ) {
	                seeker.util.set(base.targetData[i], 'show', false);
	            }
	    ...
	    panel_about.container
	        ...
	        .on('mousedown',null)
	        .html('<big><b>Annotation Viewer</b> v1.0</big><br><b>Author:</b> Damian Kao (damian.kao[at]gmail.com)<br><b>Github source:</b> <a href="https://github.com/damiankao/">https://github.com/damiankao/</a><br><b>Blog: </b><a href="http://www.nextgenetics.net">http://www.nextgenetics.net</a><br><br><b><big>Instructions:</big></b><br>1. click input to start <br>2. follow instructions on how to input your data<br>3. submit your data<br>4. manipulate the generated figure by clicking on labels, features, legend items, options<br>5. click on the export button to save as .svg<br><br>')
	        .style('top',40)
	        .style('left',50)
	        .style('width',328)
	        .style('height','auto')
	    ...
	    about_sampleButton.container
	        .style('float','right')
	        .on('click',function() {
	            var parsedData = base.parseInput(TabSampleTxt);
