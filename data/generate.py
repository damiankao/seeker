...
def newRef():
	refLen = random.randint(50000,200000)
	featureLen = random.randint(10,500)

	ref = []

	for i in range(featureLen):
		name = 'feature' + str(i).zfill(3)
		start = random.randint(1, refLen)
		endBound = start + 50000
		if endBound > refLen:
			endBound = refLen
		end = random.randint(start, endBound)

		ref.append([start,end,name])

	return ref

refs = {}
for i in range(200000):
	name = 'referenceSequence' + str(i).zfill(5)
	ref = newRef()
	refs[name] = ref
